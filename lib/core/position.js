 ...
 * @modify date 2023-01-28 15:33:03
 * @desc [属性设置相关函数]
 */
...
function setAttribLocation(gl, program, positions) {
  ...
  var resolutionUniformLocation = gl.getUniformLocation(
    program,
    "u_resolution"
  ); // 从着色器程序中查找‘u_resolution’属性的所在位置
  gl.uniform2f(resolutionUniformLocation, gl.canvas.width, gl.canvas.height); // 设置全局变量 分辨率
 ...
 */
function setAttribColor(gl, program, colorRGB) {
  var colorUniformLocation = gl.getUniformLocation(program, "u_color"); // 从着色器程序中查找‘u_color’属性的所在位置
  gl.uniform4f(colorUniformLocation, colorRGB.r, colorRGB.g, colorRGB.b, 1); // 设置一个颜色
}
...
function setRotation(gl, program, angle) {
  ...
  var rotationLocation = gl.getUniformLocation(program, "u_rotation"); // 从着色器程序中查找‘_rotation’属性的所在位置
  gl.uniform2fv(rotationLocation, rotation); // 设置旋转
}

export { setAttribLocation, setAttribColor, setTranslation, setRotation };
